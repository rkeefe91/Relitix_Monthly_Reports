...
/****** Object:  StoredProcedure [dbo].[SCWMLS_Stats_Monthly_No_CTEs]    Script Date: 5/19/2020 2:50:15 PM ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
CREATE PROCEDURE [dbo].[SCWMLS_Stats_Monthly_No_CTEs] 
...
SELECT 
cbr.Region,cbr.County,lc.Rtx_LC_ID,lc.StandardStatus,lc.ListingContractDate,lc.CloseDate,lc.ExpirationDate,lc.WithdrawnDate,lc.CancelationDate,lc.ClosePrice,lw.PendingTimestamp,'WI' As CensusState,lc.StatusChangeTimestamp
...
--(
SELECT 
cbr.Region,cbr.County,lc.Rtx_LC_ID,lc.ListingContractDate,lc.CloseDate,lc.ClosePrice,'WI' As CensusState
INTO scw_rpt_lcc
FROM Relitix.dbo.Listings_Combined lc
join
	Relitix.dbo.Listings_WIREX lw
...
from
(SELECT DISTINCT 'WI' As State,REGION,COUNTY FROM Relitix_staging.dbo.WRA_CountyByRegion_SCWMLS) c
left join
	scw_rpt_cyp cyp
on
	c.County=cyp.County
...
DROP TABLE IF EXISTS scw_rpt_cynl
DROP TABLE IF EXISTS scw_rpt_pynl
DROP TABLE Relitix.dbo.work_lci_scwmls
